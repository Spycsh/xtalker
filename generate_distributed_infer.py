...
with open("run_distributed_infer_{}.sh".format(args.slot), 'w') as f:
    ...
    for i in range(args.slot):
        ...
        f.write(f"numactl -m {socket_idx} -C {start_core}-{end_core} python3.8 inference.py --driven_audio {args.driven_audio} --source_image {args.source_image} --result_dir {args.result_dir} --cpu --rank={i} --p_num={args.slot} {dump_log} &\n")
    f.write("""wait < <(jobs -p)
rm -rf logs
""")
