...
parser.add_argument('--result_dir', type=str, default='./results', help="the directory you want to place the results")
...
parser.add_argument('--facerender', type=str, default=None, help="Face render, [pirender, facevid2vid]")
...
with open("run_distributed_infer_{}.sh".format(args.slot), 'w') as f:
    ...
    for i in range(args.slot):
        ...
        facerender = "" if not args.facerender else f"--facerender={args.facerender}"
        f.write(f"numactl -l -C {start_core}-{end_core} python inference.py --driven_audio {args.driven_audio} --source_image {args.source_image} --result_dir {args.result_dir} --cpu --rank {i} --p_num {args.slot} {bf16} {enhancer} {facerender}  {dump_log}  &\n")
    f.write("""wait < <(jobs -p)
rm -rf logs
""")
