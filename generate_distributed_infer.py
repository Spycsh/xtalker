...
with open("run_distributed_infer_{}.sh".format(args.slot), 'w') as f:
    ...
    for i in range(args.slot):
        ...
        f.write(f"numactl -m {socket_idx} -C {start_core}-{end_core} python3.8 inference.py --driven_audio andy_say_hello.wav --source_image andy_grove.jpg --result_dir ./result --cpu --rank={i} --p_num={args.slot} 2>&1|tee {i}.log &\n")
    f.write("""wait < <(jobs -p)
rm -rf logs
""")
